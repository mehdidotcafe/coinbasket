...
@mark.asyncio
async def test_native_bnb_data_source_get(snapshot, id_generator: IdGenerator):
    data_source = NativeBnbDataSource(id_generator)
    data = await data_source.get()

    assert data == snapshot


def test_native_bnb_data_source_version(id_generator: IdGenerator):
    data_source = NativeBnbDataSource(id_generator)
