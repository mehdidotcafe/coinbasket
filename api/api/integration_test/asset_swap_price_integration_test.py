...
from api.protocol.fixture.basket import test_basket, cmc20_basket
...
def test_integration_asset_swap_price_sell_basket_buy_token(snapshot):
    response = requests.post(
        ...
        cookies={"credential": credential},
        json={
            "sell_asset": test_basket.to_dict(),
            "sell_asset_amount": "1",
            "buy_asset": eth_token.to_dict(),
        },
        timeout=60,
...
def test_integration_asset_swap_price_sell_basket_buy_basket(snapshot):
    response = requests.post(
        f"http://localhost:{app_port}/asset/swap/price",
        cookies={"credential": credential},
        json={
            "sell_asset": test_basket.to_dict(),
            "sell_asset_amount": "1",
            "buy_asset": cmc20_basket.to_dict(),
        },
        timeout=60,
    )
