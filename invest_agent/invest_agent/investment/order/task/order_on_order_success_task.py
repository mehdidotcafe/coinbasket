...
from invest_agent.investment.transaction.transaction_repository import TransactionRepository
...
class OnOrderSuccessTask:
    def __init__(self, order_repository: OrderRepository, transaction_repository: TransactionRepository, posting_repository: PostingRepository, chain: Chain, date_time: DateTime, session_manager: SqlAlchemySessionManager):
    ...
    async def execute(self, order: Order, order_try: Try | None, parsed_receipt: ParsedReceipt):
        ...
        async with self.session_manager.session() as session:
            ...
            await self.transaction_repository.create_transaction(transaction, session=session)
