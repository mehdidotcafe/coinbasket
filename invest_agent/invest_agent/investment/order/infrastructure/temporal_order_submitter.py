...
class TemporalOrderSubmitter(OrderSubmitter):
    def __init__(self, order_repository: OrderRepository, id_generator: IdGenerator, configuration: Configuration):
    ...
    async def submit_orders(self, orders: list[Order]) -> list[Order]:
        ...
        result = await client.start_workflow(
            ...
            task_queue=f"order-submitter-workflow-{self.configuration['agent_name']}",
        )
    ...
    async def get_client(self):
        if self.client is None:
            self.client = await Client.connect(self.configuration["temporal_url"])
        return self.client
