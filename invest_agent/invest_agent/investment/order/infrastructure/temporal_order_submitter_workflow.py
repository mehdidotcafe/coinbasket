...
class TemporalOrderSubmitterWorkflow:
    ...
    async def run(self, orders: list[OrderRequest]):
        ...
        try:
            execute_and_wait_order_handles = [workflow.execute_child_workflow(
                TemporalExecuteAndWaitOrderWorkflow.run,
                order,
                retry_policy=retry_policy
            ) for order in orders]
            results = await asyncio.gather(*execute_and_wait_order_handles, return_exceptions=True)
            ...
            for order, result in zip(orders, results):
                if isinstance(result, BaseException):
                    ...
                    compensations.append(plan_activity(
                        "fail_order",
                        order,
            ...
            outputs.append(await workflow.execute_activity(
            ...
            outputs.append(await workflow.execute_activity(
                ...
                outputs.append(await asyncio.gather(*(c() for c in compensations), return_exceptions=True))
